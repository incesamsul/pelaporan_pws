<!-- /.navbar -->

<!-- Main Sidebar Container -->
<aside class="main-sidebar sidebar-dark-primary elevation-4">
    <!-- Brand Logo -->
    <a href="../../index3.html" class="brand-link">
        <img src="{{ asset('AdminLTE-3.2.0/dist/img/AdminLTELogo.png') }}" alt="AdminLTE Logo"
            class="brand-image img-circle elevation-3" style="opacity: .8">
        <span class="brand-text font-weight-light">Based</span>
    </a>

    <!-- Sidebar -->
    <div class="sidebar">
        <!-- Sidebar user (optional) -->
        <div class="user-panel mt-3 pb-3 mb-3 d-flex">
            <div class="image">
                <img style="width: 40px; height:40px;"
                    src="{{ auth()->user()->foto == '' ? asset('AdminLTE-3.2.0/dist/img/default-150x150.png') : asset('data/foto_profile/' . auth()->user()->foto . '/' . auth()->user()->foto) }}"
                    class="img-circle elevation-2" alt="User Image">
            </div>
            <div class="info">
                <a href="#" class="d-block">{{ auth()->user()->name }}</a>
            </div>
        </div>

        <!-- SidebarSearch Form -->
        <div class="form-inline">
            <div class="input-group" data-widget="sidebar-search">
                <input class="form-control form-control-sidebar" type="search" placeholder="Search"
                    aria-label="Search">
                <div class="input-group-append">
                    <button class="btn btn-sidebar">
                        <i class="fas fa-search fa-fw"></i>
                    </button>
                </div>
            </div>
        </div>

        <!-- Sidebar Menu -->
        <nav class="mt-2">
            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu"
                data-accordion="false">
                <!-- Add icons to the links using the .nav-icon class
               with font-awesome or any other icon font library -->

                <li class="nav-header">PENGGUNA</li>
                <li class="nav-item">
                    <a id="liDashboard" href="{{ URL::to('/dashboard') }}" class="nav-link">
                        <i class="nav-icon fas fa-tachometer-alt"></i>
                        <p>Dashboard</p>
                    </a>
                </li>
                <li class="nav-item">
                    <a id="liProfile" href="{{ URL::to('/profile') }}" class="nav-link">
                        <i class="nav-icon fas fa-user"></i>
                        <p>Profile</p>
                    </a>
                </li>
                {{-- <li class="nav-item">
                    <a id="liBantuan" href="{{ URL::to('/bantuan') }}" class="nav-link">
                        <i class="nav-icon fas fa-question-circle"></i>
                        <p>Bantuan</p>
                    </a>
                </li> --}}


                @if (auth()->user()->role == 'Administrator')
                    {{-- MENU ADMIN --}}
                    <li class="nav-header">ADMIN</li>
                    <li class="nav-item">
                        <a id="liPengguna" href="{{ URL::to('/admin/pengguna') }}" class="nav-link">
                            <i class="nav-icon fas fa-users"></i>
                            <p>Pengguna</p>
                        </a>
                    </li>
                    <li class="nav-item">
                        <a id="likabupaten" href="{{ URL::to('/admin/kabupaten') }}" class="nav-link">
                            <i class="nav-icon fas fa-map"></i>
                            <p>Kabupaten</p>
                        </a>
                    </li>


                    {{-- END OF MENU ADMIN --}}
                @endif

                @if (auth()->user()->role == 'pengelola_kabupaten')
                    <li class="nav-header">PENGELOLA KABUPATEN</li>
                    <li class="nav-item">
                        <a id="lisasaran" href="{{ URL::to('/sasaran') }}" class="nav-link">
                            <i class="nav-icon fas fa-file"></i>
                            <p>Sasaran</p>
                        </a>
                    </li>

                    <li class="nav-item">
                        <a id="liindikator" href="{{ URL::to('/indikator') }}" class="nav-link">
                            <i class="nav-icon fas fa-file"></i>
                            <p>indikator</p>
                        </a>
                    </li>
                @endif

                @if (auth()->user()->role == 'pengelola_provinsi')
                    <li class="nav-header">PENGELOLA PROVINSI</li>
                    <li class="nav-item">
                        <a id="lireportpws" href="{{ URL::to('/report_pws') }}" class="nav-link">
                            <i class="nav-icon fas fa-file"></i>
                            <p>Report Pws</p>
                        </a>
                    </li>
                @endif


                @if (auth()->user()->role == 'pimpinan')
                    <li class="nav-header">PIMPINAN</li>
                    <li class="nav-item">
                        <a id="lireportpws" href="{{ URL::to('/report_pws') }}" class="nav-link">
                            <i class="nav-icon fas fa-file"></i>
                            <p>Report Pws</p>
                        </a>
                    </li>
                @endif
                <li class="nav-header">OTHER</li>
                <li class="nav-item">
                    <a id="liPengguna" href="{{ URL::to('/logout') }}" class="nav-link">
                        <i class="nav-icon fas fa-sign-out-alt"></i>
                        <p>Logout</p>
                    </a>
                </li>
            </ul>
        </nav>
        <!-- /.sidebar-menu -->
    </div>
    <!-- /.sidebar -->
</aside>
